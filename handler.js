    ...
    try {
      ...
      let isBotAdmin = bot.isAdmin || bot.isSuperAdmin || false // Are you Admin?
        ...
        if (typeof plugin.before == 'function') if (await plugin.before.call(this, m, {
          ...
          if (xp > 99) m.reply('Chirrido -_-') // Hehehe
          else m.exp += xp
  ...
  async welcome({ m, participants }) {
    ...
    for (let user of participants) {
      ...
      try {
        ...
        let text = (chat.sWelcome || this.welcome || conn.welcome || 'Bienvenid@, @user!').replace('@user', '@' + user.split('@')[0]).replace('@subject', this.getName(m.key.remoteJid))
...
global.dfail = (type, m, conn) => {
  let msg = {
    rowner: 'Este comando solo puede ser utilizado por _*Creador*_',
    owner: 'Este comando solo puede ser utilizado por _*Propietario del Bot*_!',
    mods: 'Este comando solo puede ser utilizado por _*Moderador*_ !',
